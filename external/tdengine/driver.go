...
// quoteIdentifier 用反引号包裹标识符并转义内部反引号（TDengine）。
...
func buildInsertSQL(db string, point tsdb.SeriesPoint) string {
	...
	// Add timestamp
	columns = append(columns, quoteIdentifier("ts"))
	if point.Timestamp == 0 {
		values = append(values, "NOW()")
	} else {
		values = append(values, fmt.Sprintf("%d", point.Timestamp))
	}
...
// WritePoints writes SeriesPoints to TDengine using SQL INSERT.
